...
export class BlockchainApi {
    ...
    static rubleTransfer(fromPrivateKey:string, toPublicKey:string, amount:number) {
        return axios.post(`${baseUrl}/v1/transfers/ruble`, {
    ...
    static maticTransfer(fromPrivateKey:string, toPublicKey:string, amount:number) {
        return axios.post(`${baseUrl}/v1/transfers/matic`, {
            fromPrivateKey: fromPrivateKey,
            toPublicKey: toPublicKey,
            amount: amount
        })
     ...
     * @returns сигнатура транзакции (идентификатор транзакции в блокчейне)
     */
    static nftTransfer(fromPrivateKey:string, toPublicKey:string, amount:number) {
        return axios.post(`${baseUrl}/v1/transfers/nft`, {
            fromPrivateKey: fromPrivateKey,
            toPublicKey: toPublicKey,
            amount: amount
        })
    ...
    static transferStatus(transactionHash:string) {
        return axios.get(`${baseUrl}/v1/transfers/status`, {
            params: {
     ...
     */
     static balanceFiat(publicKey:string) {
        return axios.get(`${baseUrl}/v1/wallets/${publicKey}/balance`, {
        })
        .then(function (response : BalanceFiat) {
            return response;
        })
        .catch(function (error : any) {
            return error;
    ...
    static balanceNFT(publicKey:string) {
        return axios.get(`${baseUrl}/v1/wallets/${publicKey}/nft/balance`, {
        })
    ...
    static nftGenerate(toPublicKey:string, uri:string, nftCount:number) {
        return axios.post(`${baseUrl}/v1/nft/generate`, {
            toPublicKey: toPublicKey,
            uri: uri,
            nftCount: nftCount
        })
    ...
    static infoNFT(tokenId:number) {
        return axios.get(`${baseUrl}/v1/nft/${tokenId}`, {
        })
    ...
    static listGeneratedNFT(transactionHash:string) {
        return axios.get(`${baseUrl}/v1/nft/generate/${transactionHash}`, {
        })
     ...
     */
    static walletHistory(publicKey:string, page?:number | 100, offset?:number | 20, sort?:string | "asc") {
        return axios.post(`${baseUrl}/v1/wallets/${publicKey}/history`, {
            page: page,
            offset: offset,
            sort: sort
        })
