...
import { BalanceFiat, BalanceNFT, BlockchainApi, NFT } from "./blockchainApi";
...
export interface ITransferNftWithUsers {
  id: number;
  userId: number;
  toId: number;
  fromPrivateKey: string;
...
export interface ICreateNFTTransaction {
  userId: number;
  toId: number;
  fromPrivateKey: string;
  toPublicKey: string;
...
export class MainApi {
  ...
  static async addNftTransaction(data: ICreateNFTTransaction) {
    return axios
      .post(`${apiUrl}/transferNft`, { ...data, date: new Date() })
      .then((response) => response.data);
  }
  ...
  static async fetchAllNftTransactions() {
    const nftTransactions = axios
      .get<ITransferNFT[]>(`${apiUrl}/transferNft`)
      .then((response) => response.data);
    ...
    return Promise.all([nftTransactions, users]).then(([tNftData, uData]) => {
      return tNftData.map(
        (t) =>
          ({
            ...t,
  ...
  static async addOrder(data: IProductWithCustomer) {
    const date = new Date();

    const sum = data.count * data.product.priceRuble;
